// Get your shorts on - this is an array workout!
    ...
    const fifteen = inventors.filter(function(inventor) {
      if(inventor.year >= 1500 && inventor.year < 1600) {
        return true; // keep the inventors that have that if statement.
      }
    });
    console.table(fifteen); //returns a table in console.

    // other example - inline version
    const sixteen = inventors.filter(inventor => (inventor.year >= 1600 && inventor.year < 1900 ))
    console.table(sixteen);

    const orderByYear = sixteen.sort(function(a,b){
      if (a.year < b.year){
        return 1;
      } else {
        return -1;
    ...
    const sorted = inventors.sort((a,b) => a.year < b.year ? 1 : -1); // ternary operation
   ...
   */
    ...
    const data = ['car', 'car', 'truck', 'truck', 'train', 'bike', 'walk', 'car', 'van', 'train', 'bike', 'walk', 'car', 'van', 'car', 'truck' ];
